...
const Index = () => {
  ...
  const handleAddGoal = (newGoal: Goal) => {
    ...
    if (isEditingGoal && selectedGoalId) {
      setGoals(prevGoals => prevGoals.map(g => g.id === selectedGoalId ? {
        ...newGoal,
        id: selectedGoalId
      } : g));
    ...
    // If adding a survival goal and one already exists, update it
      ...
      setGoals(prevGoals => prevGoals.map(g => g.id === survivalGoal.id ? newGoal : g));
  ...
  const handleDeleteGoal = () => {
    ...
    setTransactions(prevTransactions => prevTransactions.filter(transaction => transaction.goalId !== goalToDelete));
  ...
  return <div className="min-h-screen bg-background">
      ...
      {/* Dialogs */}
      <AddGoalDialog isOpen={isAddGoalOpen} onClose={() => {
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteGoal} className="bg-red-500 hover:bg-red-600 text-white">
              Удалить
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
